...
contract Auction {
    ...
    function finalize() public {
        require(outcome == Outcome.SUCCESSFUL, "Can not finalize unless successful");
        require(getHighestBidder() != address(0), "Must have highest bidder to finalize");

        if (judgeAddress != address(0)) {
            require(msg.sender == judgeAddress || msg.sender == getHighestBidder(), "Only the judge or winner can finalize");
        }

        // salji sredstva prodavacu
    ...
    function refund() public {
        require(outcome == Outcome.NOT_SUCCESSFUL, "Can not refund unless not successful");
        require(getHighestBidder() != address(0), "Must have highest bidder to refund");

        if (judgeAddress != address(0)) {
            require(msg.sender == judgeAddress || msg.sender == sellerAddress, "Only the judge or seller can call refund");
        }

        // vrati sredstva kupcu 
